...
def call_model(model_cfg: dict, messages: list[dict], max_tokens: int,
    ...
    gen_s = max(1e-6, total_s - (ttft or 0))
    return {
        "ok": True,
        "text": final,
        "raw_text": text,
...
def judge_answer(judge_cfg: dict, question: str, answer: str, timeout: int) -> int | None:
    msgs = [
        {"role": "system", "content": JUDGE_SYSTEM},
        {"role": "user", "content": f"Question:\n{question}\n\nAnswer:\n{answer}\n\nScore (1-5):"},
    ]
    r = call_model(judge_cfg, msgs, max_tokens=8, temperature=0.0, timeout=timeout)
    if not r.get("ok"):
        return None
...
def write_reports(results: dict, suite: list[dict], out_root: Path) -> None:
    ...
    lines = ["# Model leaderboard", "",
             ...
             "| Model | Accuracy | Judge (1-5) | Tok/s | TTFT (s) | Fails | Peak VRAM |",
             "|---|---|---|---|---|---|---|"]
    for r in rows:
        ...
        lines.append(f"| {r['model']} | {acc} | {jdg} | {r['tok_s']} | "
                     f"{r['ttft']} | {r['fails']} | {vram} |")
    lines += ["", "Accuracy = auto-scored checkable tasks (0-1). Judge = optional "
              "local-model grade. Sort: accuracy, then speed."]
