// Stax — "Warm Campus" (Direction A) UI kit.
// Light, friendly, rounded. Built on shared.jsx primitives (Icon, Ring, Bar, Avatar, WeekDots, DSChart).
// Every component here is a candidate to graduate into a real Storybook component in the repo.
...
const DARK = {
  ...
  green: '#34C27D', greenSoft: '#15302401', amber: '#F5B53D', amberSoft: '#322811', amberInk: '#F5B53D',
...
// greenSoft dark fix (valid hex):
DARK.greenSoft = '#14302A';

const THEMES = { light: { ...BASE, ...LIGHT }, dark: { ...BASE, ...DARK } };
// Live palette object — components read A.* at render time. applyTheme() mutates it in place.
...
function Btn({ children, kind = 'primary', size = 'md', icon, iconRight, full, onClick, style }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      padding: pads, borderRadius: 14, fontFamily: A.font, fontWeight: 800, fontSize: fs,
...
function Badge({ children, tone = 'primary' }) {
  const tones = {
    primary: [A.primarySoft, A.primaryDeep], green: [A.greenSoft, A.green],
    amber: [A.amberSoft, A.amberInk], red: [A.redSoft, A.red], blue: [A.blueSoft, A.blue],
...
function Card({ children, pad = 20, style, onClick, hover }) {
  return (
    <div onClick={onClick} style={{
      background: A.card, border: `1px solid ${A.border}`, borderRadius: A.radius, padding: pad,
...
// ---- Lesson / unit card --------------------------------------------------
function LessonCard({ title, sub, icon, tint, pct, state, onClick }) {
...
function Field({ label, placeholder, value, onChange, type = 'text', icon }) {
  return (
    <label style={{ display: 'block' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: A.card, border: `1.5px solid ${A.border}`,
        ...
        <input type={type} placeholder={placeholder} value={value} onChange={onChange} style={{
          ...
          fontFamily: A.font, fontSize: 15, fontWeight: 600, color: A.ink }} />
      </div>
    </label>
  );
}
  ...
  return (
    <div onClick={onClick} style={{ width: 46, height: 27, borderRadius: 999, padding: 3, cursor: 'pointer',
      ...
      <div style={{ width: 21, height: 21, borderRadius: '50%', background: '#fff',
        transform: on ? 'translateX(19px)' : 'translateX(0)', transition: 'transform .2s ease', boxShadow: '0 1px 3px rgba(0,0,0,.2)' }}></div>
    </div>
  );
}

// ---- App shell (persistent top nav) -------------------------------------
function AppShell({ tab, go, children, maxWidth = 1180, theme = 'light', setTheme }) {
  const tabs = [
    ...
    <div style={{ background: A.page, color: A.ink, fontFamily: A.font, minHeight: '100%' }}>
      <div style={{ maxWidth, margin: '0 auto', padding: '22px 28px 60px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 26 }}>
          ...
          {tabs.map(t => (
            <div key={t.id} onClick={() => go(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 14px',
              borderRadius: 12, background: tab === t.id ? A.primarySoft : 'transparent',
              color: tab === t.id ? A.primaryDeep : A.muted, fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
              <Icon name={t.icon} size={18} />{t.label}
            </div>
          ))}
          <div style={{ flex: 1 }} />
          <Pill icon="flame" bg={A.amberSoft} fg={A.amberInk}>12</Pill>
          ...
          <div onClick={() => go('alerts')} title="Notifications" style={{ position: 'relative', width: 40, height: 40, borderRadius: 12, border: `1px solid ${A.border}`, cursor: 'pointer', display: 'grid', placeItems: 'center', color: A.muted, background: A.card }}>
            <Icon name="bell" size={19} />
            <span style={{ position: 'absolute', top: 8, right: 9, width: 7, height: 7, borderRadius: '50%', background: A.red }}></span>
          </div>
          {setTheme && (
            <div onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} title="Toggle theme" style={{
              width: 40, height: 40, borderRadius: 12, border: `1px solid ${A.border}`, cursor: 'pointer',
              display: 'grid', placeItems: 'center', color: A.muted, background: A.card }}>
...
Object.assign(window, { A, THEMES, applyTheme, Btn, Pill, Badge, Card, SectionTitle, LessonCard, Field, Toggle, AppShell });
